...
use syn::{
    ExprMatch, ExprPath, FieldPat, Member, Pat, PatOr, PatStruct, PatTupleStruct, Path, Result,
};
...
pub struct MatchAnalyzer<'a, T: CtxtForExpr> {
    /// context provider
    ctxt: &'a T,
...
impl<'a, T: CtxtForExpr> MatchAnalyzer<'a, T> {
    /// Creating a new context for analyzing a match expression
    pub fn new(ctxt: &'a T) -> Self {
        Self { ctxt, arms: vec![] }
    }
    ...
    fn get_adt_variant_by_path(&self, path: &Path) -> Result<(ADTBranch, &ADTVariant)> {
        ...
        // look-up
        let type_def = match self.ctxt.get_type(branch.type_name()) {
            None => bail_on!(path, "no such type"),
            Some(def) => def,
        };

        let variant_def = match type_def {
            TypeDef::Algebraic(adt) => match adt.variants().get(branch.variant_name()) {
    ...
    pub fn into_organized(
        ...
        if self.arms.is_empty() {
            bail_on!(expr, "no arms in match");
        }
        let ref_ty = self.arms.first().unwrap().body.ty().clone();
        for item in &self.arms {
            if item.body.ty() != &ref_ty {
                bail_on!(expr, "match arm type mismatch");
            }
        }
                ...
                match atom {
                    ...
                    MatchAtom::Binding(binding) => {
                        for branch in binding.keys() {
                            if adt_name != branch.type_name() {
                                bail_on!(expr, "atoms and heads ADT name mismatch");
                            }
                            if !adt_variants.contains(branch.variant_name()) {
                        ...
                        MatchAtom::Binding(binding) => match binding.get(combo_branch) {
                            ...
                            Some(unpack) => {
                                let variant = MatchVariant {
                                    adt: combo_branch.type_name().clone(),
                                    branch: combo_branch.variant_name().to_string(),
                                    unpack: unpack.clone(),
                                };
